...
vec3 getTerrainInfoLONG(vec2 pos) {
...
vec3 getTerrainInfo(vec2 worldPos) {
	vec2 samplingCoord = (worldPos / samplingScale) + vec2(0.5);
	//vec3 terrainInfo = texture(terrainImage, samplingCoord).xyz;
	vec3 terrainInfo = getTerrainInfoLONG(worldPos);
	terrainInfo.yz /= samplingScale;
	if (true) {
		float h = 0.001;
		terrainInfo.y = (getTerrainInfoLONG(vec2(worldPos.x + h, worldPos.y)).x - terrainInfo.x) / h;
		terrainInfo.z = (getTerrainInfoLONG(vec2(worldPos.x, worldPos.y + h)).x - terrainInfo.x) / h;
	}
	return terrainInfo;
}
