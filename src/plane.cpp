...
Plane::Plane(int width, int vertexDensity, const glm::vec3& cameraPosition, float latticeSize)
	: mWidth{ width }
{
	assert(!(width == 1 && vertexDensity == 1));
	...
	for (int vertexI{ 0 }; vertexI < vertexCount; ++vertexI) {
		...
		vertices[vertexI * floatsPerVertex + 2] = Random::getFloat();
		vertices[vertexI * floatsPerVertex + 3] = Random::getFloat();
		vertices[vertexI * floatsPerVertex + 4] = Random::getFloat();
...
void Plane::Rebuild(float width, int vertexDensity) {
